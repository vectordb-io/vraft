  ...
  if (req.url === '/') {
    fs.readdir(directoryPath, (err, files) => {
      ...
      const otherFiles = files.filter(
          file => file.endsWith('.sm') || file.endsWith('.log') ||
              file.endsWith('.message') || file.endsWith('.global'));

      const generateLinks = (files) => {
        return files
            .map(file => `<a href="${file}" target="_blank">${file}</a>`)
            .join('<br>');
      };
      ...
      fs.readFile(filePath, (err, data) => {
        ...
        if (filePath.endsWith('.html')) {
          ...
          contentType = 'image/svg+xml';
            ...
            filePath.endsWith('.sm') || filePath.endsWith('.log') ||
            filePath.endsWith('.message') || filePath.endsWith('.global')) {
