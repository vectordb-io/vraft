...
# 生成数据流图
cat ${dir}/remu.log.sm | grep -E "event_send|event_timer|event_start|event_stop| propose-value| leader-transfer| enable-recv| enable-send| disable-recv| disable-send| increase-term| step-down| become-leader" | grep -v "heartbeat-timer" > ${dir}/temp/remu.log.sm.df.1
